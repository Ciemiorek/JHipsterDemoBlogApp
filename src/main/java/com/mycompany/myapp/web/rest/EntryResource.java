...
public class EntryResource {
    ...
    public ResponseEntity<?> createEntry(@Valid @RequestBody Entry entry) throws URISyntaxException {
        ...
        if (entry.getBlog() != null && !entry.getBlog().getUser().getLogin().equals(SecurityUtils.getCurrentUserLogin().orElse(""))){
    ...
    public ResponseEntity<?> getEntry(@PathVariable Long id) {
        ...
        if (entry.isPresent() && entry.get().getBlog()!=null && !(entry.get().getBlog().getUser().getLogin().equals(SecurityUtils.getCurrentUserLogin().orElse("")))){
